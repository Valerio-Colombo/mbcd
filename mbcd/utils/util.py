...
def kl_mvn(m0, S0, m1, S1):
    ...
    det_term  = np.log(det_S1/det_S0) # np.sum(np.log(S1)) - np.sum(np.log(S0))
    quad_term = diff.T @ np.linalg.inv(S1) @ diff # np.sum( (diff*diff) * iS1, axis=1)
    # print(tr_term,det_term,quad_term)
    return .5 * (tr_term + det_term + quad_term - N)


def kl_mvn_diagonal(m0, S0, m1, S1):
    """
    Kullback-Liebler divergence from Gaussian pm,pv to Gaussian qm,qv.
    Also computes KL divergence from a single Gaussian pm,pv to a set
    of Gaussians qm,qv.
    ...
    # covariance matrices are passed as vectors representing the diagonals
    iS1 = np.reciprocal(S1)  # S^-1
    diff = m1 - m0

    # KL is made of three terms
    tr_term = np.sum(iS1*S0)
    det_term = np.sum(np.log(S1)) - np.sum(np.log(S0))
    quad_term = np.sum(diff * iS1 * diff)
...
def _KL_test(m0, S0, m1, S1):
    ...
    # kl is made of three terms
    tr_term = np.trace(iS1 @ S0)
    det_term = np.sum(np.log(np.diagonal(S1))) - np.sum(np.log(np.diagonal(S0)))
    quad_term = np.sum(diff*np.diagonal(iS1)*diff)
